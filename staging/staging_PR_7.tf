...
# changes.
#
resource "random_id" "server_PR_7" {
  keepers = {
...
resource "aws_instance" "staging_cicd_demo_PR_7" {
  # Read the AMI id "through" the random_id resource to ensure that
  # both will change together.
  ami                    = random_id.server_PR_7.keepers.ami_id
  ...
  EOF
  tags = {
    "Name" = "staging_cicd_demo-PR_7"
  }
}

output "staging_dns_PR_7" {
  value = aws_instance.staging_cicd_demo_PR_7.public_dns
}
