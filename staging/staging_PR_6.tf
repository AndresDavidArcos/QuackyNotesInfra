...
# changes.
#
resource "random_id" "server_PR_6" {
  keepers = {
...
resource "aws_instance" "staging_cicd_demo_PR_6" {
  # Read the AMI id "through" the random_id resource to ensure that
  # both will change together.
  ami                    = random_id.server_PR_6.keepers.ami_id
  ...
  EOF
  tags = {
    "Name" = "staging_cicd_demo-PR_6"
  }
}

output "staging_dns_PR_6" {
  value = aws_instance.staging_cicd_demo_PR_6.public_dns
}
