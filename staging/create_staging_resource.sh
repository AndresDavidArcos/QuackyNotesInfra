...
_main() {
  ...
  if [[ ! -f "${local_dir}/staging_${resource_id}.tf" ]]; then
    # Create a copy of the staging.tf.example file template
    cp \
      "${local_dir}/staging.tf.example" \
      "${local_dir}/staging_${resource_id}.tf"

    # Replace the <RANDOMID> with the actual random identifier
    sed -i "s/RANDOMID/${resource_id}/g" "${local_dir}/staging_${resource_id}.tf"
  fi

  # Exit after outputing the meta data either way
  echo '{"resource_file": "staging_'${resource_id}'.tf", "terraform_expected_output": "staging_dns_'${resource_id}'"}'
  exit 0
}
