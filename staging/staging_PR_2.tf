...
# changes.
#
resource "random_id" "server_PR_2" {
  keepers = {
...
resource "aws_instance" "staging_cicd_demo_PR_2" {
  # Read the AMI id "through" the random_id resource to ensure that
  # both will change together.
  ami                    = random_id.server_PR_2.keepers.ami_id
  ...
  EOF
  tags = {
    "Name" = "staging_cicd_demo-PR_2"
  }
}

output "staging_dns_PR_2" {
  value = aws_instance.staging_cicd_demo_PR_2.public_dns
}
